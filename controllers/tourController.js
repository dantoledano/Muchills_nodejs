...
exports.resizeTourImages = catchAsync(async (req, res, next) => {
  if (!req.files.imageCover || !req.files.images) {
    //return next(new AppError('Please upload images for cover and at least one image for gallery', 400));
    return next();
  }
...
exports.getToursWithin = catchAsync(async (req, res, next) => {
  ...
  if (!lat || !lng) {
    next(
      new AppError(
        'Please provide a latitude and longitude in hte format lat,lng.',
        400,
      ),
    );
  }
